...
_(Pipe &amp; Filter Architectural Pattern)_
...
3. OpenNLPStemmer _(uses the OpenNLP Porter stemmer)_
4. PorterStemmer _(uses the Porter stemmer version given by the instructor)_
5. StopWordRemover
...
| --- | --- |
| SENTINEL\_VALUE | A string that is used to signal the end of the text stream. |
| PIPE\_CAPACITY | The buffer size of the pipes (same for all) |
| STOP\_WORDS | An array of stop words |
